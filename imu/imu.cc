...
bool IMU::Init(int i2cfd) {
  ...
  // config gyro
  i2c_write(fd_, ADDR_ITG3200, 0x3E, 0x01);  // use X gyro PLL oscillator
  i2c_write(fd_, ADDR_ITG3200, 0x15, 19);    // samplerate 50Hz (1000/(19+1))
  i2c_write(fd_, ADDR_ITG3200, 0x16, 0x18 + 4);  // enable, 20Hz bandwidth
  // config compass
  i2c_write(fd_, ADDR_HMC5883L, 0x00, 0x38);  // CRA: 75Hz rate w/ 2 averages
  i2c_write(fd_, ADDR_HMC5883L, 0x01, 0x20);  // CRB: set gain 1090 LSB/Gauss
  i2c_write(fd_, ADDR_HMC5883L, 0x02, 0x00);  // continuous measurement
  // config accelerometer
  i2c_write(fd_, ADDR_ADXL345, 0x2c, 0x09);  // 25Hz bw, 50Hz samplerate
  i2c_write(fd_, ADDR_ADXL345, 0x31, 0x08);  // FULL_RES
  i2c_write(fd_, ADDR_ADXL345, 0x38, 0x00);  // bypass FIFO, sample @ 50Hz
  i2c_write(fd_, ADDR_ADXL345, 0x2d, 0x08);  // turn on
...
bool IMU::ReadRaw(IMURawState *s) {
  ...
  uint8_t axis_buf[8];
  if (i2c_read(fd_, ADDR_ITG3200, 0x1b, axis_buf, 8))
    return false;
  ...
  if (i2c_read(fd_, ADDR_HMC5883L, 0x03, axis_buf, 6))
    return false;
  ...
  if (i2c_read(fd_, ADDR_ADXL345, 0x32, axis_buf, 6))
    return false;
